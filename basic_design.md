...
- **タイトル & 状態表示 & トレース準備**: `PyTrace - トレース学習支援システム` タイトル、実行準備状態バッジ（準備完了/初期化中/Python・マクロ言語コード変更検知等）、およびその右隣に配置された強調ボタン「トレース準備」。マクロ言語編集時も即座に変更を検知してボタンが活性化し、ワンクリックでPython逆変換・トレース再実行が可能。
- **ステップナビゲーション**: 「最初」「前へ」「次へ」「最後」ボタンを集中配置。
...
- **トグル式タブバー**:
  ...
  - **統合コントロール群**:
    - **言語種別バッジ**: 選択中サンプルの言語（`Python` / `マクロ言語`）を示すカラーバッジをプルダウン左隣に表示。
    - **サンプル選択プルダウン**: サンプル名が見やすいワイド幅（`minWidth: 220px`）。サンプル選択時はサンプルの言語種別に応じて対応するコード画面（Python / マクロ言語）へ自動遷移。
    - **拡大率スライダー**: 50%〜400% のズームバーを最右端に配置。
- **スマート自動相互変換 & 流れ図同期**:
  - **リアルタイム変更検知 & トレース準備連携**: Pythonだけでなくマクロ言語の直接編集時も即座に変更を検知し、マクロ言語タブを開いたまま「トレース準備」を実行可能（自動でPython逆変換・トレース実行を行い「準備完了」に復帰）。
...
- **コード(Python)エディタ**:
  - **Monaco Editor**（Python シンタックスハイライト、直接編集）。
  - **フローティング「.py読込」ボタン**: Python タブ表示時のみ、エディタ右上に半透明ブラー背景の `.py 読込` ボタンを表示（他タブ選択時は非表示）。
  - 実行行のリアルタイム・デコレーションハイライト。
  - **余白最適化**: グリフマージン無効化（`glyphMargin: false`）、行番号最小文字幅の3桁化（`lineNumbersMinChars: 3`）により左端余白をスリム化（折りたたみ機能 `folding: true` は維持）。
- **コード(マクロ言語)エディタ**:
  - **Monaco Editor**（VBA シンタックスハイライト、全構文キーワードの青色表示、直接編集）。
  - Python トレース実行行と連動したリアルタイム・デコレーションハイライト。
  - **余白最適化**: Pythonエディタと同様に左端余白をスリム化。
- **流れ図ビューア**:
  - Python AST から自動生成された標準流れ図（SVG 独自レンダリング）。
  - 現在実行中のノードの自動ハイライト。
  - タブバーのズームスライダーによる拡大縮小表示（ドラッグパン対応）。
  - **フローティング「💾 draw.io形式で書き出し」ボタン**: 流れ図エリア右下にアクセントボタンを配置。クリックでファイル保存ダイアログを開き、diagrams.net（draw.io）で直接編集可能な `.drawio` XML ファイルとして保存可能。
- **作業領域の最大化**:
...
- **スプレッドシート型 変数履歴表**:
  ...
  - **「現在の値」固定行（Sticky Header）**: テーブルヘッダー2行目に「現在の値」を常時固定表示。ステップ進行に伴う各変数の最新値やローカル変数バッジ（`L`）を即座に確認可能。
  - **コード変更時自動クリア**: コードが変更され未準備（not ready）となった際、自動で表をクリアして「表示する変数の履歴がありません」を表示。
...
graph TD
    ...
    A --> I["vbaConverter.ts (Python <-> VBA)"]
    A --> J["drawioExporter.ts (.drawio XML Export)"]
```
...
- **if / elif / else 多分岐**:
  ...
  - 合流線の矢印先端は、すっきりとした線のみの開いた形状（オープン矢印 `>`、開き角度約60度、線幅1.0）を描画。
- **行末コメントのアノテーション表示**:
