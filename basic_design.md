...
- **タイトル & 状態表示 & トレース準備**: `PyTrace - トレース学習支援システム` タイトル、実行準備状態バッジ（準備完了/初期化中/変更検知等）、およびその右隣に配置された強調ボタン「トレース準備」。
...
- **トグル式タブバー**:
  ...
  - **統合コントロール群**:
    ...
    - **サンプル選択プルダウン**: サンプル名が見やすいワイド幅（`maxWidth: 210px`）。サンプル選択時はサンプルの言語種別に応じて対応するコード画面（Python / マクロ言語）へ自動遷移。
